...
import { Trash2, X, ToggleLeft, ToggleRight, ChevronLeft, Plus } from "lucide-react";
  ...
  return (
    <div className="container-mobile fade-up">
      ...
      <div className="text-center mb-12">
        ...
        <h2 className="text-4xl font-extrabold tracking-tighter text-indigo-400">{formatMoney(totalActivoARS)}</h2>
      </div>

      <div className="space-y-4">
        {gastos.map((g) => {
          return (
            <div key={g._id} className={`premium-card transition-all ${!g.activo ? "opacity-30" : ""} ${editId === g._id ? "p-6 bg-[#151515] ring-1 ring-primary/20" : ""}`}>
              {editId === g._id ? (
                <div className="space-y-4">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[9px] font-bold uppercase tracking-widest text-primary">Editar servicio</span>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleEdit(g._id)} className="flex-1 btn-primary py-2.5 text-[9px]">Guardar</button>
                    <button onClick={() => handleDelete(g._id)} className="p-2.5 bg-danger/10 text-danger rounded-xl"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    <button onClick={() => handleToggle(g._id, g.activo)} className="shrink-0 active:scale-90 transition-all text-primary">
                      {g.activo ? <ToggleRight className="w-10 h-10" /> : <ToggleLeft className="w-10 h-10 text-muted/20" />}
                    </button>
                    <div className="flex-1 min-w-0" onClick={() => setEditId(g._id)}>
                      <p className="font-bold truncate text-sm text-white/90">{g.nombre}</p>
                      <p className="text-[9px] font-bold text-muted uppercase tracking-widest mt-0.5">Vence: Día {g.diaVencimiento} · {g.categoria}</p>
                    </div>
                  ...
                  {g.activo && (
                    <div className="flex gap-1 overflow-x-auto no-scrollbar pt-2 border-t border-white/5">
                      {MESES_SHORT.map((m, i) => {
                        const isPaid = g.pagos?.some((p) => p.mes === i && p.anio === currentYear);
                        const isCurrent = i === currentMonth;
                        
                        return (
                          <button key={i} onClick={() => handleTogglePago(g._id, i, currentYear)}
                            className={`month-dot ${
                              isPaid ? "bg-success text-black border-success" : 

  ...
  return (
    <div className="container-mobile fade-in-up">
      <div className="flex items-center gap-8 mb-20">
        ...
        <h1 className="text-4xl font-extrabold tracking-tighter">Tarjetas</h1>
      </div>

      <div className="text-center mb-20">
        <p className="text-[10px] font-bold text-muted uppercase tracking-[0.4em] mb-4">Capital Pendiente</p>
        <h2 className="text-5xl font-extrabold tracking-tighter">{formatMoney(totalPendiente)}</h2>
      </div>
      ...
      <div className="space-y-8">
        {activas.map((c) => {
          ...
          return (
            <div key={c._id} className="premium-card p-10 space-y-10 group">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <h3 className="text-2xl font-bold tracking-tight mb-2">{c.descripcion}</h3>
                  <p className="text-[10px] font-bold text-muted uppercase tracking-widest">{c.tarjeta} · {c.categoria}</p>
                </div>
                <button onClick={() => handleDelete(c._id)} className="p-3 text-muted/20 hover:text-danger active:scale-90 transition-all">
              ...
              <div className="space-y-4">
                <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest">
                  <span className="text-white">Pago {c.cuotasPagadas} / {c.cantidadCuotas}</span>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-white/5">
                <div>
                  ...
                  <p className="text-xl font-bold">{formatMoney(restante)}</p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => handleDeshacer(c._id, c.cuotasPagadas)}
                    className="p-5 rounded-2xl bg-[#111111] text-muted active:scale-90 transition-all disabled:opacity-10"
                    disabled={c.cuotasPagadas <= 0}>
                    <Minus className="w-5 h-5" />
                  </button>
                  <button onClick={() => handlePagar(c._id, c.cuotasPagadas)}
                    className="p-5 rounded-2xl bg-white text-black active:scale-90 transition-all disabled:opacity-10"
        ...
        {activas.length === 0 && (
          <div className="p-24 text-center">
            <CardIcon className="w-12 h-12 text-muted/20 mx-auto mb-6" />
            <p className="text-xs font-bold text-muted uppercase tracking-widest">Sin deudas activas</p>
          </div>
        )}
      </div>
    </div>
